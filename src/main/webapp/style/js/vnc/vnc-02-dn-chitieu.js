...
$(document).ready(function() {
	...
	/*eventTable.loadDataTest('vnc02-treetable', arrayTest2);*/
	/*
	 * $.contextMenu({ selector : '.context-menu-vnc02', callback :
	 * function(key, options) { switch (key) { case "addone": var rowChild = {
	 * "chiTieu" : "Thủy sản " , "donVI" : "triệu đồng", }
	 * 
	 * var rownumber = table.rows().data().length; alert("Ok"+rownumber); break;
	 * case "addchild": alert("Wanna add child? " + $(this).attr('id')) break;
	 * case "delete": alert("Wanna delete? " + $(this).attr('id')) break; } },
	 * items : { "addone" : { name : "Thêm chỉ tiêu", icon : "edit" },
	 * "addchild" : { name : "Thêm chỉ tiêu con", icon : "edit" }, "delete" : {
	 * name : "Xóa khỏi danh sách", icon : "delete" }, } });
	 */
	...
	clickModal : function(idButton, positain, id) {
		$('#' + idButton).on('click', function() {
			...
			if (positain == 'addone') {
				...
				// LOAD TABLE AGAIN
				//Tong
				/*var tongIDRoot = arrayTest.length;
				var tongallnode = tongIDRoot;
				for ( var i=0;i<tongIDRoot;i++){
					tongallnode += arrayTest[i]['child'].length;
					for (var y=0;y<arrayTest[i]['child'].length;y++){
						tongallnode += arrayTest[i]['child'][y]['grand'].length;
					}
				}
				var idRoot = findIDRoot(arrayTest,tongIDRoot, id);
				var idChild = findIDChild(arrayTest,tongIDRoot, id);
...
/*function findIDRoot(arrayTest,tongIDRoot,id){
	
	Find rootid
	var idroot;
	for ( var i=0;i<tongIDRoot;i++){
		id--;
		id -= arrayTest[i]['child'].length;
		if(id <= 0){
			return i;
		}
		for (var y=0;y<arrayTest[i]['child'].length;y++){
			id -= arrayTest[i]['child'][y]['grand'].length;
			if(id <= 0)
			{
				return i;
			}
...
function findIDChild(arrayTest,tongIDRoot,id){
