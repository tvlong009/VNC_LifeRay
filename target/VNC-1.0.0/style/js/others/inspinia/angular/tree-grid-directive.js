(function () {
  angular
    .module('tree-grid-template', [])
    .run([
      '$templateCache',
      function ($templateCache) {
        $templateCache.put('tree-grid-template',          
          ...
          "       <td>" +
          "				<i ng-class=\"row.tree_first_icon\"\n" +
          "              ng-click=\"row.branch.expanded = !row.branch.expanded\"\n" +
          "              class=\"indented tree-icon\"></i>\n" +
          "				<span class=\"indented tree-label\" ng-click=\"on_user_click(row.branch)\">\n" +
          "             {{row.branch[expandingProperty.field] || row.branch[expandingProperty]}}</span>\n" +	
          "				<a ng-click=\"user_clicks_branch(row.branch)\" class=\"a-ng-click\">" +
          "					<i ng-class=\"row.tree_last_icon\"\n" +
          "              	ng-click=\"row.branch.expanded = !row.branch.expanded\"\n" +
          "              	class=\"indented tree-icon\"></i>\n" +
          "           	</a>\n"+			
          "       </td>\n" +
  ...
  angular
      ...
      function ($timeout,
        ...
        return {
          ...
          link       : function (scope, element, attrs) {
            ...
            attrs.building = attrs.building ? attrs.building : 'fa fa-building';
            attrs.users = attrs.users ? attrs.users : 'fa fa-users';
            attrs.user = attrs.user ? attrs.user : 'fa fa-user';
            ...
            on_treeData_change = function () {
              ...
              add_branch_to_list = function (level, branch, visible) {
                var child, child_visible, tree_last_icon, tree_first_icon, _i, _len, _ref, _results;
                if (branch.expanded == null) {
                  branch.expanded = false;
                }
                if (!branch.children || branch.children.length === 0) {
                	tree_last_icon = attrs.iconLeaf;
                } else {
                  if (branch.expanded) {
                	  tree_last_icon = attrs.iconCollapse;
                  } else {
                	  tree_last_icon = attrs.iconExpand;
                  }
                }
                
                if (level == 1) {
                	tree_first_icon = attrs.building;
                } else if (level == 2) {
                	tree_first_icon = attrs.users;
                } else {
                	tree_first_icon = attrs.user;
                }
                ...
                scope.tree_rows.push({
                  ...
                  tree_last_icon: tree_last_icon,
                  tree_first_icon: tree_first_icon,
                  visible  : visible
                });
                if (branch.children != null) {
                  _ref = branch.children;
    ...
    .provider('treegridTemplate', function () {
      var templatePath = 'tree-grid-template';
